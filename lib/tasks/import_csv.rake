...
namespace :import do
  ...
  task :emission_tests, [:file] => :environment do |t, args|
    ...
    default_drive_type = DriveType.find_by(name: 'Electric') || DriveType.first
    success_count = 0
    error_count   = 0
    errors        = []
    CSV.foreach(file_path, headers: true, encoding: 'bom|utf-8') do |row|
      begin
        ...
        vehicle = Vehicle.find_or_create_by!(folder_code: vehicle_serial) do |v|
          v.description   = row['Vehicle #'].to_s.strip
          v.model         = row['Vehicle Model'].to_s.strip
          v.location      = location
          v.engine_config = engine_config
          v.drive_type    = default_drive_type
        end
        test_date = begin
