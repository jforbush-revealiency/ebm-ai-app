module Api
  class DiagnosticController < BaseController
    ...
    def show
      ...
      # Thresholds (multiplicative — applied against rated values)
      very_low_nox     = params_hash['Very_Low_NOx'].to_f        # -0.25
      low_nox          = params_hash['Low_NOx'].to_f             # -0.20
      nox_upper_max    = params_hash['Nox_Upper_Max'].to_f       # 0.20
      elevated_co2     = params_hash['Elevated_CO2_Percentage'].to_f  # 0.0025
      high_co2         = params_hash['High_CO2_Percentage'].to_f      # 0.05
      elevated_co_mult = params_hash['Elevated_CO'].to_f         # 2.0
      high_co_mult     = params_hash['High_CO'].to_f             # 3.0
      high_co_low_nox  = params_hash['High_CO_with_Low_NOx'].to_f    # 0.80
      bank_check_max   = params_hash['Bank_Check_Max'].to_f      # 0.20
      rpm_max          = params_hash['Rated_RPM_Max'].to_f       # 0.10
      rpm_min          = params_hash['Rated_RPM_Min'].to_f       # -0.20
      ...
      is_single_stack = vehicle.engine_config.engine&.is_single_stack
      ...
      # HP check
      hp_result = diagnose_hp(input.engine_hp, input.alternator_hp, rated_hp, params_hash)

      # Per-bank diagnostics
      ...
      render json: {
        ...
        test_type:   input.auto_generated? ? 'telematic' : 'manual',
        ...
        sections: {
          ...
          hp:            hp_result,
          co2: {
            left:  left_co2_result,
            right: right_co2_result
          },
          co: {
            left:  left_co_result,
            right: right_co_result
          },
          nox: {
            left:  left_nox_result,
            right: right_nox_result
          },
    ...
    def diagnose_engine_hours(input, p)
      # Just report — no pass/fail threshold for hours
      { status: 'ok', value: input.engine_hours, message: p['Message_ok_engine_hours'] || 'OK' }
    end
    ...
    def diagnose_rpm(actual_rpm, rated_rpm, rpm_min, rpm_max, p)
      ...
      if ratio > rpm_max
        { status: 'warning', value: actual_rpm, message: p['Message_rated_rpm_limit_exceeded'] }
      elsif ratio < rpm_min
        { status: 'warning', value: actual_rpm, message: p['Message_investigate_engine_alternator_rpm_settings'] }
      else
        { status: 'ok', value: actual_rpm, message: p['Message_ok_engine_alternator_rpm_settings'] || 'OK' }
      end
    end

    def diagnose_hp(engine_hp, alternator_hp, rated_hp, p)
      return { status: 'unknown', message: 'No HP data' } if engine_hp.nil? || rated_hp.zero?
      ratio = (engine_hp.to_f - rated_hp) / rated_hp
      hp_variance_max = p['Horse_Power_Variances_Max'].to_f
      if ratio.abs > hp_variance_max
        { status: 'warning', value: engine_hp, message: p['Message_check_engine_hp_settings'] }
      else
        { status: 'ok', value: engine_hp, message: p['Message_ok_engine_alternator_hp_settings'] || 'OK' }
      end
    end

    def diagnose_co2(actual_co2, rated_co2, elevated_co2, high_co2, p)
      return { status: 'unknown', message: 'No CO2 data' } if actual_co2.zero?
      ratio = (actual_co2 - rated_co2) / rated_co2
      if ratio > high_co2
        { status: 'warning', value: actual_co2, message: p['Message_high_co2_percentage'] }
      elsif ratio > elevated_co2
        { status: 'caution', value: actual_co2, message: p['Message_elevated_co2_percentage'] }
      elsif ratio < -high_co2
        { status: 'caution', value: actual_co2, message: p['Message_low_co2_percentage'] }
      else
        { status: 'ok', value: actual_co2, message: p['Message_ok_co2_percentage_banks'] || 'OK' }
      end
    end

    def diagnose_co(actual_co, rated_co, actual_nox, rated_nox, elevated_mult, high_mult, high_co_low_nox, p)
      return { status: 'skip', message: 'CO not measured (telematic)' } if actual_co.zero?
      nox_ratio = rated_nox.zero? ? 0 : (actual_nox - rated_nox) / rated_nox
      if actual_co >= rated_co * high_mult
        { status: 'warning', value: actual_co, message: p['Message_extremely_high_co'] }
      elsif actual_co >= rated_co * elevated_mult && nox_ratio < high_co_low_nox
        { status: 'warning', value: actual_co, message: p['Message_high_co_with_low_nox'] }
      elsif actual_co >= rated_co * elevated_mult
        { status: 'caution', value: actual_co, message: p['Message_high_co'] }
      elsif actual_co > rated_co
        { status: 'notice', value: actual_co, message: p['Message_elevated_co'] }
      else
        { status: 'ok', value: actual_co, message: p['Message_ok_co_banks'] || 'OK' }
      end
    end

    def diagnose_nox(actual_nox, rated_nox, very_low_nox, low_nox, nox_upper_max, p)
      return { status: 'unknown', message: 'No NOx data' } if actual_nox.zero?
      ratio = (actual_nox - rated_nox) / rated_nox
      if ratio < very_low_nox
        { status: 'warning', value: actual_nox, message: p['Message_very_low_nox'] }
      elsif ratio < low_nox
        { status: 'caution', value: actual_nox, message: p['Message_low_nox'] }
      elsif ratio > nox_upper_max
        { status: 'warning', value: actual_nox, message: p['Message_high_nox'] }
      else
        { status: 'ok', value: actual_nox, message: p['Message_ok_nox_banks'] || 'OK' }
      end
    end

    def diagnose_bank_balance(left_nox, right_nox, left_co2, right_co2, bank_check_max, p)
      nox_variance  = rated_nox_variance(left_nox, right_nox)
      co2_variance  = rated_nox_variance(left_co2, right_co2)
      if nox_variance > bank_check_max || co2_variance > bank_check_max
        { status: 'warning', message: p['Message_check_left_right_bank_performance'] }
      else
        { status: 'ok', message: p['Message_ok_bank_balance_check'] || 'OK' }
      end
    end

    def rated_nox_variance(left, right)
      avg = (left + right) / 2.0
